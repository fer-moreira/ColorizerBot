...
class DeepLearning (object):
    ...
    def build (self):
        if self.output_path == "": raise FileNotFoundError
        if ((self.resolution_x + self.resolution_y) / 2) < 10: raise ValueIsTooSmallError("Minimum Resolution 200x200 ")
        if ((self.resolution_x + self.resolution_y) / 2) > 2000: raise ValueIsTooSmallError("Maximum Resolution 5000x5000")

        logging.info("Building...")

        # Load Data
        self.numpy_file = self.numpy_file.transpose().reshape(self.shape_x, self.shape_y, 1, 1)
